  ...
  const paymentTableByYear = (() => {
    ...
    return Object.entries(byYear)
      .map(([year, months]) => {
        ...
        const scheduled = months.reduce(
          (sum, m) => sum + getScheduledForMonth(m.month),
          0
        );
        const diff = totalPaid - scheduled;
        return {
  ...
  return (
    <Box sx={{ mb: 4 }}>
        ...
        >
          ...
          <Tooltip title="Record actual payments made. Overpayments reduce interest and tenure.">
            <IconButton
              ...
              sx={{ ml: 1 }}
              tabIndex={0}
              aria-label="Actual payment info"
            >
        ...
        <Grid container spacing={2} alignItems="center" justifyContent="center">
          <Grid xs={12} sm={3}>
            <TextField
              ...
              error={
                !!error && (!entry.month || isNaN(parseInt(entry.month, 10)))
              }
              helperText={
                !!error && (!entry.month || isNaN(parseInt(entry.month, 10)))
                  ? error
          ...
          <Grid xs={12} sm={4}>
            <TextField
              label={
                <span>
                  Payment Amount{" "}
                  <Tooltip title="Overpayments reduce interest and tenure">
                    <IconButton size="small">
                      <InfoOutlinedIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
              ...
              sx={{ background: "#F9FAFB", borderRadius: 1 }}
              error={
                !!error && (!entry.amount || isNaN(parseFloat(entry.amount)))
              }
              helperText={
                !!error && (!entry.amount || isNaN(parseFloat(entry.amount)))
                  ? error
      ...
      <Paper
        ...
        elevation={3}
        className="card actual-payment-container"
      >
        <Typography
          variant="h5"
          fontWeight={700}
      ...
      >
        <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
          <Button
            variant={historyView === "month" ? "contained" : "outlined"}
            onClick={() => setHistoryView("month")}
          >
            By Month
          </Button>
          <Button
            variant={historyView === "year" ? "contained" : "outlined"}
                    ...
                    >
                      <td
                        style={{
                          ...
                          borderRadius:
                            expandedMonth === row.month ? "8px 0 0 8px" : 0,
                        }}
                      >
                        {row.month}
                      </td>
